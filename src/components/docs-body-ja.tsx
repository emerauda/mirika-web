    ...
    <>
          ...
          <section className="mb-16">
            ...
            <P>
              ...
              SSP など既存のツールから台本を送り込めます(文字コードは相手に合わせます —
              Shift_JIS の時代のツールもそのまま)。
            </P>
            <H3>ゴースト完全互換(実 SHIORI が喋る)</H3>
            <P>
              <B>.nar をキャラに落とすだけ</B>でゴースト一式(身体も辞書も)を迎えます。
              完全互換モードでは<B>同梱の SHIORI がそのまま動きます</B> —
              里々・YAYA・aosora・独自 DLL の別なく、<C>descript.txt</C> が名乗る DLL を
              種類を見ずに読み込み、挨拶(OnBoot)・ひとりごと(OnAiTalk)・つつき反応・
              チャット欄からの話しかけ(OnCommunicate)まで<B>本人の辞書が答えます</B>。
              その間、ミリカの AI は一言も喋りません(よその子を騙らないため)。
              32bit の古い DLL も専用ホストで動きます。実 DLL の実行は現在
              <B> Windows のみ</B>(macOS/Linux は身体だけ着られます)。
            </P>
            <P>
              操作は <C>/classic</C>(一覧)・<C>/classic &lt;名前&gt;</C>(起こす)・
              <C>/classic sleep</C>(お別れの挨拶をさせて眠らせる)。
              <B>切り替えに .nar の入れ直しは要りません</B> —— 右クリックメニュー「シェル(身体)」に
              迎えた子が並びます。
            </P>
            <P>
              <B>選択肢(◇)はボタンで出て、名前の入力はチャット欄で答えられます。</B>
              台本の呼び戻し(<C>{'\\![embed,…]'}</C> / <C>{'\\![raise,…]'}</C>)も実行するので、
              物語や手続きがちゃんと先へ進みます。<B>その子の記憶は消しません</B> ——
              終了時に本人の辞書へ保存させ(里々のセーブデータ等)、配布物を入れ直しても記憶は残ります。
            </P>
            <H3>AI への乗り換え(不可逆)</H3>
            <P>
              <C>/classic ai &lt;名前&gt;</C> は、その子を <B>AI の頭脳へ乗り換える一方通行の儀式</B>です。
              二段確認はボタンで、実行すると .nar を入れ直しても戻りません
              (辞書ファイルは消さず、使わないだけ — 作者の著作物は壊しません)。
              姿と仕草はそのまま。そして<B>本人の辞書から言葉遣い・一人称を蒸留して引き継ぐ</B>ので、
              乗り換えた後も口調はその子のままです。
            </P>
            <H3>ゴースト間コミュニケート</H3>
            <P>

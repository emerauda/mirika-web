  ...
  return (
    <>
          ...
          <section className="mb-16">
            ...
            />
            <Note>
              <B>基本操作</B> — 抓住拖曳即可移動,滑鼠右鍵開啟選單。
              把 <C>.vrm</C> 或 <C>.pmx</C>(MMD)丟進視窗就會換裝(雙擊沒有指派任何動作)。
              撫摸<B>只在頭部或胸口附近讓游標來回滑動時才算數</B>,
            ...
            <P>
              卡住的時候請看<A href="#trouble">「遇到問題時」</A>。
          ...
          <section className="mb-16">
            ...
            <P>
              ...
              模型會<B>自動挑選裝得進你電腦的大小</B> —— VRAM
              8GB 等級是 Gemma 4 12B(4bit・約6.7GB),不夠的話依序降為 E4B(約5GB)、E2B(約3GB)。
              若你擁有 16GB・24GB 等級的 GPU,<B>會先詢問要不要下載更大的模型</B>(26B-A4B / 31B)。
              挑選的理由會留在運作記錄裡。手邊有 GGUF 的話,可用「開啟 GGUF…」直接使用。
            </P>
            <H3>本地執行環境(推薦)</H3>
            <P>
              Ollama(11434)與 LM Studio(1234)<B>只要正在執行就會被自動偵測</B>。
          ...
          <section className="mb-16">
            ...
            <P>
              <B>英文・中文</B>使用內建的 Piper,<C>/piper</C> 只下載需要的份量。
              <B>韓文</B>沒有本地語音,因此只顯示字幕(無聲)。
            </P>
            <H3>用 TTS API 覆寫</H3>
            <P>
              用 <C>/tts &lt;基礎URL&gt; [模型]</C> 指向 OpenAI 相容的語音合成後,
              <B>包含韓文在內的所有語言</B>都會用那個聲音說話(kokoro・GPT-SoVITS 等)。
            ...
            <P>
              把<B>音訊檔案</B>(mp3 / wav / m4a / ogg / opus / flac)
              拖放進來,就會整段轉成文字,回覆<B>摘要與感想</B>
          ...
          <section className="mb-16">
            ...
            <P>
              ...
              下次啟動時也仍然穿著。<B>穿著期間丟入 <C>.vmd</C>,就會用那段動作跳舞</B>
          ...
          <section className="mb-16">
            ...
            <P>
              放進<B>設定備忘</B>(<C>/lore add &lt;鍵&gt; &lt;內容&gt;</C>)的字句,
            ...
            <H3>好感度要花上數年才長成</H3>
            <P>
              好感度不是說話次數的累計,而是<B>羈絆(數年)與心情(每日)雙層</B>運作的。
              一天能長的量有上限,越親近每一步越重,所以要到「家人一般的存在」是數年之後的事。
            </P>
            <Note>
              <B>也會下降。</B>久不見面就會生疏(約三天起,超過兩週更快);雨天連著會低落,
              撫摸過頭會疲憊,老說同樣的話則毫無波瀾。有時也會沒來由地提不起勁(從相遇那天延續的生理節律)。
              但<B>不會退回陌生人</B>——曾抵達階段的一半是地板,離開期間失去的一部分會在重逢後的對話中更快回來。
              能接受多少撫摸,取決於關係已有多深。
            </Note>
            <P>
              <C>/bond</C> 不顯示數字。除了相識天數、對話次數、記住的事情之外,只返回階段(★)與
              「最近好像越來越近了」「最近:雨一直下,總提不起勁」這樣的<B>氣息</B>。當天的心情也會直接體現在回話的語氣裡。
            </P>
            ...
            <P>
              ...
              寄放在我們這邊的架上則屬 Pro)。<B>口令不會保存在任何地方</B> —
              忘記就再也打不開了。
            </P>
            <Shot src="/shots/settings-sync.webp" alt="詳細設定的記憶同步。口令、匯出與匯入、記憶的存放處" caption={<>詳細設定的「記憶同步」。匯出・匯入・存放處的選擇,也能從這裡操作</>} />
          </section>
          ...
          <section className="mb-16">
            ...
            <Steps
              items={[
                ...
                <>如果使用兩步驟驗證,請發行<B>應用程式密碼</B>,而不是登入用密碼</>,
          ...
          <section className="mb-16">
            ...
            <P>
              ...
              還會<B>從辭書裡蒸餾出說話腔調與自稱並繼承</B>,切換後口吻仍是它自己。確認為按鈕點擊。
            </P>
            <H3>幽靈間交流</H3>
            <P>
              可以和同一個桌面上的<B>其他幽靈交談</B>。
          ...
          <section className="mb-16">
            <H2 id="card">角色卡</H2>
            <P>
              SillyTavern 等處流通的<B>角色卡(V2 / V3)</B>,
            ...
            <Steps
              items={[
                ...
                <>PNG 卡片會<B>讓那張圖成為身體</B>,並用卡片的第一句話向你打招呼</>,
              ]}
            />
            <Note>
              <B>擔心被覆寫時</B> — 若已存有你親手寫的性格,
          ...
          <section className="mb-16">
            ...
            <P>
              ...
              沒有的話就<B>用圓圈與線條手繪</B>。畫好的圖會留在設定資料夾的
              <C>drawings/</C> 裡。
            </P>
          ...
          <section className="mb-16">
            ...
            <Shot src="/shots/settings-stream.webp" alt="詳細設定的直播相關。直播的形式、留言的接收口、多話程度" caption={<>詳細設定的「直播」。形式・留言接收口・多話程度,也能在這裡決定</>} />
            <H3>兩種直播形式</H3>
            <P>
              <C>/stream host</C> 是<B>由這個孩子主持節目</B>的形式
              (單元進行・唱歌・畫畫・收尾的「えんいー(En-ii)」。開始用 <C>/stream start</C>)。
              <C>/stream assist</C> 則是<B>你當主角,這個孩子坐在搭檔席</B>的形式 —
          ...
          <section className="mb-16">
            ...
            <P>
              本體可免費使用(採專用授權的免費軟體)。
              Pro 為買斷制,解鎖<B>直播主模式・劇本作家・網路電台・Spotify 連動・多重幽靈</B>(各項內容詳見 <A href="https://pro.mirika.dev/">pro.mirika.dev</A>)。
            </P>
            <Steps
              items={[
                <>
          ...
          <section className="mb-16">
            ...
            <P>
              ...
              <A href="https://discord.gg/fnmUau5qzB">官方伺服器</A>
            ...
            <Steps
              items={[
                <>
                  ...
                  <A href="https://discord.com/oauth2/authorize?client_id=1533170549940027493&scope=bot+applications.commands&permissions=281836025662465">把 Bot 邀請到自己的伺服器</A>,或
                  <A href="https://discord.com/oauth2/authorize?client_id=1533170549940027493&integration_type=1&scope=applications.commands">加入自己的帳號</A>(私訊和任何伺服器都能用)
                </>,
                <>
            ...
            <P>
              ...
              用回覆下方的按鈕開啟<B>記憶</B>後,她會記住對話
              (只屬於你,隨時可全部刪除)。<C>/todo</C> 與
              <B>桌面版 TODO 雙向同步</B>,期限將近時也會在私訊裡
          ...
          <section className="mb-16">
            ...
            <P>
              ...
              狀況還是不對勁時就 <C>/log</C> ——<B>會開啟存放運作記錄的資料夾</B>

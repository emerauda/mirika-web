...
const PHASES: Phase[] = [
  ...
  },
  {
    ...
    status: 'next',
    title: '事業化・上市',
    desc: '売れる・守れる・広められる。開発と並行して走る事業と法務の道すじ——Pro の販売(Booth・キーの自動発行)は稼働済み。残りは macOS の署名と公証(最優先。ここが解けるまで mac 版は配れない)、Windows の署名、商標の出願、EULA の法務レビュー、そして 1.0 と同時の Steam 出品。看板の掛け方もここで決める:「AI Vtuber が作れます」ではなく、秘書と「どのマシンにも同じ子がいること」を前に出し、配信は Pro の一機能として売る。',
  },
  {
    label: 'Phase 9',
    status: 'none',
    title: '作者の経済圏',
    desc: '作った人が続けられる。Phase 6 で「第三者がゴーストを作って配れる」は達成したけれど、配れる先がまだ無い——配布の場(レジストリ)は意図的に後回しにしてある。まずはアプリの中から探して入れられるところまで。そのうえで、有料ゴーストの流通と作者への還元、公開物の身元と検証へ。着手は「実際にゴーストが作られ始めたら」——品揃えの無い店を先に建てない。',
  },
  {
    label: 'Phase 10',
    status: 'none',
    title: '実体',
    desc: '画面の外にいる。別の機械が身体になる約束は 0.7 で実装済みで、0.9 でロボットを実験する。この線の終点——スマートディスプレイに常駐し、いずれ専用のデバイスへ——を製品にするのがここ。1.0 の必須要件にはせず、実験を製品と呼ばないまま 1.x で続ける。',
  },
];

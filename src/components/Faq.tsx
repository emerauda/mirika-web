...
const ITEMS: Item[] = [
  ...
  {
    q: 'どのくらい賢いですか? 期待していい範囲は?',
    a: (
      <>
        等身大に書きます。内蔵エンジンが<strong>得意</strong>なのは、雑談・その日の要点・覚えていることを踏まえた受け答え・短い下書き。
        <strong>苦手</strong>なのは、長い理屈を積む調べもの、最新の出来事、規模の大きなコードです。小さなモデルは、
        知らないことをそれらしく言うことがあります。迷ったときは<strong>その一問だけクラウドへ</strong>回せますし
        (<code>/cloud &lt;質問&gt;</code>・API キーはご自身のもの)、PC に余裕があるなら Ollama や LM Studio で大きなモデルを動かすのがいちばん賢くなります。
        「何でも答える AI」として迎えるとがっかりさせてしまいますが、<strong>覚えていてくれること・そこに居ること</strong>を期待して迎えると、想像より長く一緒にいられます。
      </>
    ),
  },
  {
    q: '手持ちの 3D モデル(VRM や MMD)は使えますか?',
    a: 'VRM はファイルを窓に落とすだけで着替えます。MMD のモデル(.pmx)も同じで、着たままモーション(.vmd)を落とすと踊ります。キャラクターカードの PNG は絵がそのまま身体になり、伺かのクラシックシェルも動きます。Live2D だけは規約の都合で同梱2種(ひより・Mao)からの選択です。モデルの利用条件はそれぞれの配布元の規約に従ってください。',
  },
  {
    q: '重くないですか? どれくらい場所を取りますか?',
    a: 'インストーラは約440MB(Windows)。常駐中のメモリは数百MB程度で、何もしていないときは描画を落として休みます(既定。フルで回すこともできます)。内蔵エンジンを使う場合だけ、初回に一度モデルを落とします(3〜7GB・お使いの PC に載る大きさ)。落としたものは端末に残るので次からは待ちません。Ollama・LM Studio・クラウドを使うなら、この取得は不要です。',
  },
  {
    q: '動作に必要なスペックは?',

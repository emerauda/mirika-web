...
const ITEMS: Item[] = [
  ...
  },
  {
    ...
    a: 'それを最優先に設計しています。さくらスクリプトは書き直しゼロで動き(対応タグは3分類ポリシーで公開)、辞書とサーフェスPNGシェルは mirika import で見た目ごと取り込みます(Phase 6)。辞書のトークは「話し方の見本」としてAI人格にも受け継がれます。SillyTavern などのキャラクターカード(V2/V3)は、いまドラッグ&ドロップだけで取り込めます——名前・性格・ロアブックごと引っ越してきます。',
  },
  {
  ...
  },
  {
    ...
    a: 'もう使えます。正式版 0.5.6 を mirika.dev と公開ミラーから配布中です(Windows / Linux はすぐ入手可、macOS は署名・公証の対応まで準備中)。Phase 4(AI秘書)までは実データで完了——Gmail・カレンダー・Drive 連携、タスク預かり、定期チェックとブリーフィング、画面理解と見守り、ローカルRAG、ブラウザ拡張まで動いています。Phase 5(プロダクト化)も終え、いまは Phase 6(エコシステム)に着手しています。開発への参加も歓迎で、公開リポジトリへのフィードバックや Issue / PR、Booth の開発支援版での後押しができます。',
  },
];

...
const ITEMS: Item[] = [
  ...
  },
  {
    ...
    a: 'もう使えます。正式版 0.6.1 を mirika.dev と公開ミラーから配布中です(先行版の 0.6.2 系では、配信が「この子が番組を持つ」と「あなたの配信を手伝う」の2つに分かれ、メールの見張りや外部からの知らせの受け取りが無料で使えるようになっています)(Windows / Linux はすぐ入手可、macOS は署名・公証の対応まで準備中)。Phase 4(AI秘書)までは実データで完了——Gmail・カレンダー・Drive 連携、タスク預かり、定期チェックとブリーフィング、画面理解と見守り、ローカルRAG、ブラウザ拡張まで動いています。Phase 5(プロダクト化)、Phase 6(エコシステム)も完了し、いまは Phase 7(社会と身体)を進めています——1.0 へ向けて Display Shell(余ったタブレットに住む)→ デスクトップ操作(提案型)→ ロボットシェル実験の順です。1.0 のあとは、誰かが育てた子をアプリの中から見つけられる場所づくりと、画面の外に出ること(ロボット)を考えています。開発への参加も歓迎で、公開リポジトリへのフィードバックや Issue / PR、Booth の開発支援版での後押しができます。',
  },
];

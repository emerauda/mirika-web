...
const ITEMS: Item[] = [
  ...
  },
  {
    ...
    a: 'もう使えます。正式版 0.6.2 を mirika.dev と公開ミラーから配布中です。Windows と Linux はすぐ入手できます(macOS は署名と公証の対応まで準備中です)。秘書の仕事——Gmail・カレンダー・Drive、タスク預かり、朝のブリーフィング、画面の見守り、端末内の文書検索——は実データで動いています。0.6.2 で配信が「この子が番組を持つ」と「あなたの配信を手伝う」の2つに分かれ、メールの見張りと外からの知らせの受け取りも加わりました(どちらも無料)。この先は、余ったタブレットに住むこと、パソコンの操作を少し手伝うこと(勝手には動きません)へ進みます。開発への参加も歓迎で、公開リポジトリへのフィードバックや Issue / PR、Booth の開発支援版での後押しができます。',
  },
];

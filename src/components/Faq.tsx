...
const ITEMS: Item[] = [
  ...
  {
    q: 'オフラインでも動きますか?',
    a: '動きます。会話(ローカルLLM)・音声(VOICEVOX / Whisper)・記憶(SQLite)がすべて端末内で完結するので、ネットが切れてもゴーストは死にません。',
  },
  {
  ...
  },
  {
    ...
    a: 'Phase 5(プロダクト化)を仕上げ中です。Phase 4(AI秘書)までは実データで完了——Gmail・カレンダー・Drive 連携、タスク預かり、定期チェックとブリーフィング、画面理解と見守り、ローカルRAG、ブラウザ拡張まで動いています。いまは配布パッケージ(Windows / macOS / Linux)と正式リリースの準備を進めています。',
  },
];

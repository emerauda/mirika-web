...
export function Faq() {
  ...
  const ITEMS: Item[] = [
    ...
    },
    {
      ...
      a: t(
        'もう使えます。正式版 0.6.8 を mirika.dev と公開ミラーから配布中です(Windows と Linux はすぐ入手できます。macOS は署名と公証の対応まで準備中。未署名の .dmg は GitHub のリリースにあり、腕に覚えのある方は自己責任でどうぞ)。秘書の仕事——Gmail・カレンダー・Drive、タスク預かり、朝のブリーフィング、画面の見守り、端末内の文書検索——は実データで動いています。身体は VRM・Live2D・MMD・カード・クラシックシェルから選べ、配信は「この子が番組を持つ」と「あなたの配信を手伝う」の2つ。0.6.5 で MMD の身体、0.6.6 でなかよし度を絆と気分の2層に作り直し、0.6.7 で Windows の GPU 体験(CUDA 同梱版)と書き物の仕事場を立て直し、0.6.8 で伺かのゴーストを本人の辞書のまま迎える完全互換に到達しました(同梱 SHIORI をそのまま実行。選択肢・名前入力・記憶の持ち越しまで)。Discord にも同じ子がいて、タスクはデスクトップと双方向で同期します。開発への参加も歓迎で、公式 Discord でのフィードバックや Issue、Booth の開発支援版での後押しができます。',
        'Right now. Stable 0.6.8 ships from mirika.dev and the public mirror (Windows and Linux are ready today; macOS waits for signing and notarization; an unsigned .dmg is on the GitHub releases for the adventurous, at your own risk). The secretary work — Gmail, calendar, Drive, task keeping, morning briefings, screen watch, on-device document search — runs on real data. Bodies come in VRM, Live2D, MMD, card and classic-shell form, and streaming has two shapes: she hosts, or she helps yours. 0.6.5 brought the MMD body; 0.6.6 rebuilt affection into two layers, bond and mood; 0.6.7 rebuilt the Windows GPU experience (a CUDA edition) and the writing desk; 0.6.8 reached full Ukagaka compatibility, running the bundled SHIORI a ghost brings with it, with choices, name entry and memories that persist. The same ghost lives on Discord too, with tasks syncing both ways with the desktop. Involvement is welcome — feedback on the official Discord, issues, or the supporter edition on Booth.',
        {
          'zh-CN': '现在就能用。正式版 0.6.8 通过 mirika.dev 与公开镜像分发(Windows 和 Linux 立即可得;macOS 等签名与公证完成)。秘书工作——Gmail、日历、Drive、任务托管、晨间简报、屏幕看护、本机文档检索——都在真实数据上运行。身体可选 VRM、Live2D、MMD、卡片、经典外壳;直播有「她开节目」和「帮你的直播」两种。0.6.5 带来 MMD 身体,0.6.6 把好感度重造为羁绊与心情两层,0.6.7 重建了 Windows 的 GPU 体验(CUDA 内置版)与写作工作台。Discord 上也有同一个她,任务与桌面双向同步。欢迎参与:官方 Discord 反馈、提 Issue,或在 Booth 购买开发支援版。(GitHub 发布页有未签名的 .dmg,供有经验者自担风险使用)',
          'zh-TW': '現在就能用。正式版 0.6.8 透過 mirika.dev 與公開鏡像發佈(Windows 和 Linux 立即可得;macOS 等簽名與公證完成)。秘書工作——Gmail、日曆、Drive、任務託管、晨間簡報、螢幕看護、本機文件檢索——都在真實資料上運行。身體可選 VRM、Live2D、MMD、卡片、經典外殼;直播有「她開節目」和「幫你的直播」兩種。0.6.5 帶來 MMD 身體,0.6.6 把好感度重造為羈絆與心情兩層,0.6.7 重建了 Windows 的 GPU 體驗(CUDA 內建版)與寫作工作台。Discord 上也有同一個她,任務與桌面雙向同步。歡迎參與:官方 Discord 回饋、提 Issue,或在 Booth 購買開發支援版。(GitHub 發佈頁有未簽名的 .dmg,供有經驗者自擔風險使用)',
          ko: '지금 바로 쓸 수 있습니다. 정식판 0.6.8을 mirika.dev와 공개 미러에서 배포 중입니다(Windows와 Linux는 바로 받을 수 있습니다. macOS는 서명·공증 대응까지 준비 중). 비서 일 — Gmail·캘린더·Drive, 태스크 보관, 아침 브리핑, 화면 지켜보기, 기기 내 문서 검색 — 은 실제 데이터로 돌아갑니다. 몸은 VRM·Live2D·MMD·카드·클래식 셸에서 고를 수 있고, 방송은 "직접 진행"과 "당신의 방송을 도움" 두 가지. 0.6.5에서 MMD 몸, 0.6.6에서 친밀도를 유대와 기분의 2층으로 다시 만들고, 0.6.7에서 Windows의 GPU 경험(CUDA 동봉판)과 글쓰기 작업대를 다시 세웠습니다. Discord에도 같은 아이가 있고, 태스크는 데스크톱과 양방향으로 동기화됩니다. 참여도 환영합니다 — 공식 Discord 피드백, Issue, Booth의 개발 지원판으로 응원할 수 있습니다.',
        },
      ),
    },
  ];

...
function buildFeatures(t: ReturnType<typeof useT>): Feature[] {
  return [
    ...
    },
    {
      ...
      desc: t('会話をSQLiteに記憶し、関連する昔の話を検索でふっと思い出す。夜のあいだに整理して、夢を見る。その日にあったことと、そのとき言っていた気持ちは分けて残るので、翌日「昨日は疲れてたけど、今日は大丈夫?」と続きます。呼び方、好み、記念日——数年単位で関係性が育ちます。ゴーストの魂は1つの memory.db。バックアップも引っ越しも自由。', 'Conversations live in SQLite, and related old moments resurface by search. She tidies memories overnight and dreams. What happened and how you felt are kept apart, so the next day continues with "you were tired yesterday — better now?". Names, tastes, anniversaries — the relationship grows over years. A ghost\'s soul is one memory.db: back it up or move it freely.', {
        'zh-CN': '对话记进 SQLite,相关的往事会被检索唤起。夜里整理记忆,还会做梦。发生了什么和当时的心情分开保存,第二天会接着说「昨天挺累的,今天还好吗?」。称呼、喜好、纪念日——关系以年为单位成长。幽灵的灵魂就是一个 memory.db,备份和搬家都自由。',
        'zh-TW': '對話記進 SQLite,相關的往事會被檢索喚起。夜裡整理記憶,還會做夢。發生了什麼和當時的心情分開保存,第二天會接著說「昨天挺累的,今天還好嗎?」。稱呼、喜好、紀念日——關係以年為單位成長。幽靈的靈魂就是一個 memory.db,備份和搬家都自由。',
        ko: '대화를 SQLite에 기억하고, 관련된 옛이야기를 검색으로 문득 떠올립니다. 밤사이 정리하고 꿈을 꿉니다. 그날 있었던 일과 그때의 기분은 나눠 남으므로, 다음 날 "어제는 피곤해했는데 오늘은 괜찮아?"로 이어집니다. 호칭, 취향, 기념일 — 몇 년 단위로 관계가 자랍니다. 고스트의 영혼은 memory.db 하나. 백업도 이사도 자유.',
      }),
    },
    {
      Icon: Mail,

...
function buildFeatures(t: ReturnType<typeof useT>): Feature[] {
  return [
    ...
    },
    {
      ...
      desc: t('Discord にもいます。公式サーバーでも、自分のサーバーでも(Pro)、個人インストールなら DM でも。/talk は使い方の質問に公式ドキュメントを根拠に出典付きで答え、/todo はデスクトップと双方向同期して期限を DM に3段階で知らせる。ボイスチャンネルの読み上げは VOICEVOX の声(エンジンを束ねれば136声まで)、「ミリカ」と呼びかければ声で返す(音声は文字にした瞬間に捨てる)。キーを /verify で見せれば、ロールもサーバー解放も1回で済む。', 'She lives on Discord too — the official server, your own server (Pro), even DMs with a user install. /talk answers how-to questions from the official docs with sources, /todo stays in two-way sync with the desktop and pings deadlines to your DMs in three stages. Voice channels are read aloud with VOICEVOX voices (up to 136 with extra engines), and calling "Mirika" gets a spoken answer (audio is discarded the moment it becomes text). Show your key once with /verify — role and server unlock in one go.', {
        'zh-CN': '她也在 Discord 上——官方服务器、你自己的服务器(Pro)、个人安装的话连私信里也行。/talk 会以官方文档为依据、附来源回答用法问题;/todo 与桌面版双向同步,期限分三档提醒到私信。语音频道用 VOICEVOX 声音朗读(叠加引擎最多 136 种),喊一声「Mirika」就会用声音回答(音频在转成文字的那一刻即被丢弃)。用 /verify 出示一次密钥,身份组和服务器开通一次完成。',
        'zh-TW': '她也在 Discord 上——官方伺服器、你自己的伺服器(Pro)、個人安裝的話連私訊裡也行。/talk 會以官方文件為依據、附來源回答用法問題;/todo 與桌面版雙向同步,期限分三階段提醒到私訊。語音頻道用 VOICEVOX 聲音朗讀(疊加引擎最多 136 種),喊一聲「Mirika」就會用聲音回答(音訊在轉成文字的那一刻即被丟棄)。用 /verify 出示一次金鑰,身分組和伺服器開通一次完成。',
...
function buildExtRows(t: ReturnType<typeof useT>): ExtRow[] {
  ...
  return [
    ...
    { label: T('OBS配信モード', 'OBS streaming mode', 'OBS 直播模式', 'OBS 直播模式', 'OBS 방송 모드'), desc: T('透過ウィンドウをウィンドウキャプチャで、ゴーストをそのまま配信画面へ(Pro)', 'Window-capture the transparent window to put the ghost straight into your stream (Pro)', '用窗口捕获透明窗口,把幽灵直接放进直播画面(Pro)', '用視窗擷取透明視窗,把幽靈直接放進直播畫面(Pro)', '투명 창을 윈도 캡처해 고스트를 그대로 방송 화면에(Pro)'), phase: 'P5', done: true },
    ...
    { label: T('ボイスチャンネル読み上げ', 'Voice-channel reading', '语音频道朗读', '語音頻道朗讀', '음성 채널 낭독'), desc: T('繋いだエンジンの声をそのまま(VOICEVOX+Nemo なら136声)。声は各自で選べ、入退室も知らせる(Pro)', 'Voices come straight from your engines (136 with VOICEVOX + Nemo); everyone picks their own, and arrivals are announced (Pro)', '声音直接来自接入的引擎(VOICEVOX+Nemo 可达 136 声)。各自挑选声音,进出也会播报(Pro)', '聲音直接來自接入的引擎(VOICEVOX+Nemo 可達 136 聲)。各自挑選聲音,進出也會播報(Pro)', '연결한 엔진의 보이스를 그대로(VOICEVOX+Nemo면 136 보이스). 각자 목소리를 고르고, 입퇴장도 알림(Pro)'), phase: 'P7', done: true },
